  ...
  return (
    ...
    >
      ...
      <Box variant="small" color="text-body-secondary" padding={{ top: 's' }}>
        ...
        chunks (P1&ndash;P3) that share each step&apos;s token budget with R1&apos;s decodes; R3 joins the batch the step it
        arrives; R4 waits while the budget is saturated, then joins once R1 finishes and frees space. The batch&apos;s
        membership changes every single step &mdash; that is continuous (iteration-level) batching. Illustrative schedule,
        not measured output.
      </Box>
    </div>
  );
  ...
  return (
    <SpaceBetween size="l">
      ...
      >
        <SpaceBetween size="m">
          <Box variant="p">
            <strong>The problem:</strong> Static batching &mdash; collect N requests, run them
            together until <em>all</em> finish, then start the next batch &mdash; wastes the GPU.
          ...
          <Box variant="p">
            ...
            &mdash; all without interrupting the requests that are mid-generation. The GPU stays
        ...
        <SpaceBetween size="m">
          ...
          <ColumnLayout columns={3} variant="text-grid">
            <div>
              ...
              <Box variant="p">
                R3 arrives at t2 and is scheduled the same step &mdash; it does not wait for R1 or
            ...
            <div>
              ...
              <Box variant="p">
                ...
                immediately reusable &mdash; which is exactly what lets R4 finally get scheduled.
              </Box>
            </div>
            <div>
              <Box variant="h3">Prefill is chunked and interleaved</Box>
              <Box variant="p">
                R2&apos;s long prompt is split into chunks P1&ndash;P3 spread across t0&ndash;t2, sharing
      ...
      <Container header={<Header variant="h2">The Unified Scheduler &amp; the Per-Step Token Budget</Header>}>
        <SpaceBetween size="m">
          <Box variant="p">
            ...
            <strong>unified scheduler</strong> &ldquo;treats both prompt and output tokens the same way by
          ...
          <Box variant="p">
            Mechanically, each step the scheduler is handed a <strong>token budget</strong> &mdash;{' '}
            <code>max_num_batched_tokens</code> &mdash; and a cap on how many requests can be in the
            batch &mdash; <code>max_num_seqs</code>. It then fills the budget by assigning a token
          ...
          <ColumnLayout columns={2} variant="text-grid">
            <div>
              ...
              <Box variant="p">
                ...
                throughput-vs-latency dial. &ldquo;Smaller values (e.g., 2048) achieve better inter-token
                latency (ITL) because there are fewer prefills slowing down decodes.&rdquo; &ldquo;Higher values
            ...
            <div>
              ...
              <Box variant="p">
                ...
                consumes more KV cache; lowering it &ldquo;reduces the number of concurrent requests in a
                batch, thereby requiring less KV cache space.&rdquo;{' '}
                <Link
                  external
                  href="https://docs.vllm.ai/en/stable/configuration/optimization.html"
                  ariaLabel="vLLM optimization guide, accessed 2026-06-07"
          ...
          <Alert type="info">
            ...
            <code>max_num_batched_tokens &gt; 8192</code> &mdash; &ldquo;especially for smaller models on
            large GPUs.&rdquo; The trade-off is real: large batches push more prefill tokens per step,
      ...
      <Container header={<Header variant="h2">Chunked Prefill</Header>}>
        <SpaceBetween size="m">
          <Box variant="p">
            ...
            request in the batch is stalled &mdash; classic head-of-line blocking that spikes
          ...
          <Box variant="p">
            In V1 this is the default: &ldquo;Chunked prefill is enabled by default whenever possible,
            unlike in V0 where it was conditionally enabled based on model characteristics.&rdquo;{' '}
            <Link
              external
              href="https://docs.vllm.ai/en/stable/usage/v1_guide/"
              ariaLabel="vLLM V1 user guide, accessed 2026-06-07"
            ...
            It works by giving prefill chunks and decode tokens a shared per-step budget, &ldquo;better
            balancing compute-bound (prefill) and memory-bound (decode) operations.&rdquo;{' '}
            <Link
              external
              href="https://docs.vllm.ai/en/stable/configuration/optimization.html"
              ariaLabel="vLLM optimization guide, accessed 2026-06-07"
          ...
          <Alert type="warning">
            <strong>Critical caveat &mdash; chunked prefill does NOT reduce KV-cache memory.</strong>{' '}
            ...
            you &mdash; the constraint is KV-cache capacity (covered in{' '}
            ...
            a compute-scheduling/latency feature; the &ldquo;no KV reduction&rdquo; framing is the architectural
            consequence &mdash; KV footprint is a function of sequence length, not chunk size &mdash; not a
      ...
      <Container header={<Header variant="h2">Preemption Under Memory Pressure</Header>}>
        <SpaceBetween size="m">
          <Box variant="p">
            Continuous batching admits requests aggressively to keep the GPU busy &mdash; which means
            the scheduler can over-commit KV cache. As accepted requests grow their output, &ldquo;there
            are times when KV cache space is insufficient to handle all batched requests.&rdquo; When that
          ...
          <Box variant="p">
            &ldquo;In vLLM V1, the default preemption mode is <code>RECOMPUTE</code> rather than{' '}
            <code>SWAP</code>.&rdquo; RECOMPUTE discards the preempted request&apos;s KV cache entirely and
            re-runs its prefill from scratch when it is rescheduled &mdash; trading wasted compute for
          ...
          <Alert type="error">
            <strong>Preemption is a throughput killer &mdash; treat repeated preemptions as an alarm.</strong>{' '}
          ...
          <Box variant="p">
            ...
            recommend: &ldquo;Increase <code>gpu_memory_utilization</code>&rdquo; (give the KV cache more of the
            GPU&apos;s memory); &ldquo;Decrease <code>max_num_seqs</code> or <code>max_num_batched_tokens</code>&rdquo;
            (admit fewer concurrent requests so they fit); &ldquo;Increase <code>tensor_parallel_size</code>&rdquo;
            or &ldquo;Increase <code>pipeline_parallel_size</code>&rdquo; (shard the model across more GPUs so the
      ...
      <Container header={<Header variant="h2">Tuning Quick Reference</Header>}>
        <SpaceBetween size="m">
          ...
          <Alert type="info">
            ...
            capacity, not by these knobs alone &mdash; see{' '}
            ...
            &mdash; the budget loop, the running/waiting queues, and how preemption is wired &mdash;
            see the <strong>&ldquo;Inside the Codebase &rarr; Scheduler &amp; KV Cache&rdquo;</strong> tab.
          </Alert>
        </SpaceBetween>
      </Container>
          ...
          <ExpandableSection headerText="Why iteration-level scheduling beats request-level scheduling">
            <SpaceBetween size="s">
              ...
              <Box variant="p">
                ...
                operations) so it does not eat the throughput it unlocks. The actual implementation
                lives in the scheduler module &mdash; see the codebase tab.
              </Box>
            </SpaceBetween>
          </ExpandableSection>
          <ExpandableSection headerText="How the per-step budget interleaves prefill and decode">
            <SpaceBetween size="s">
              <Box variant="p">
                ...
                mix &mdash; the &ldquo;unified&rdquo; in unified scheduler. This is also why
          ...
          <ExpandableSection headerText="RECOMPUTE vs SWAP — why V1 dropped swapping">
            <SpaceBetween size="s">
              <Box variant="p">
                ...
                means wasted work &mdash; the right move is to avoid it via the memory knobs above,
